...
main() {
  test('Schema from relative filesystem URI should be supported', () async {
    ...
    final schema =
        await JsonSchema.createFromUrl('test/relative_refs/root.json');

    expect(schema.validate({"string": 123, "integer": 123}).isValid, isFalse);
    expect(
        schema.validate({"string": "a string", "integer": "a string"}).isValid,
        isFalse);
    expect(schema.validate({"string": "a string", "integer": 123}).isValid,
        isTrue);
  });
}
